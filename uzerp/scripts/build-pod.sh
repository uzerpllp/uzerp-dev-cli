...
podman run --pod uzerp-pod --name uzerp-app-dev --security-opt label=disable --security-opt apparmor=unconfined \
-v $UZERP_SOURCE_DIR:/var/www/html:rw \
--env XDEBUG_CONFIG="${XDEBUG_CONFIG}" \
-d quay.io/uzerp/uzerp-app-dev

podman run --pod uzerp-pod --name uzerp-frepple --security-opt label=disable --security-opt apparmor=unconfined \
